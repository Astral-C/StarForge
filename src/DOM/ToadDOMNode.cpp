...
void SToadDOMNode::RenderHeirarchyUI(std::shared_ptr<SDOMNodeBase>& selected){
	...
	if(selected == GetSharedPtr<SToadDOMNode>(EDOMNodeType::Object)){
		ImGui::TextColored(ImColor(0,255,0), std::format("{0} Toad", ToadColorNames[glm::max(mObjArgs[0], 0)]).c_str());
	} else if(selected == mLinkedObject.lock()) {
		ImGui::TextColored(ImColor(0,255,150), std::format("{0} [Linked]", mName.data()).c_str());
	} else {
		ImGui::Text(std::format("{0} Toad", ToadColorNames[glm::max(mObjArgs[1], 0)]).c_str());
	}
